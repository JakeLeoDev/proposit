			...
			>
				...
				>
					...
					{message.parts?.map((part, index) => {
										...
										<MessageContent
											className={cn({
												'w-fit break-words rounded-2xl px-3 py-2 text-right text-white':
													message.role === 'user',
												'bg-transparent px-0 py-0 text-left rounded-none': message.role === 'assistant',
											})}
						...
						// Handle tool calls - check if type starts with "tool-"
						if (typeof type === 'string' && type.startsWith('tool-')) {
							const toolPart = part as any;
							const { toolCallId, state } = toolPart;

							return (
								<Tool className="w-full min-w-0" defaultOpen={false} key={toolCallId || `tool-${index}`}>
									<ToolHeader
										state={state}
									...
									<ToolContent>
										...
										{state === 'output-available' && (
											<ToolOutput
												...
												output={
													toolPart.output ? (
														<div className="min-w-0 rounded border p-2 break-words">
										...
										{state === 'output-error' && (
											<ToolOutput errorText={toolPart.errorText || 'An error occurred'} output={undefined} />
										)}
									</ToolContent>
								</Tool>
							);
